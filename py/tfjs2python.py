...
for x in variables:
    ...
    d = np.reshape(d, variables[x]['shape'])
...
def convToOutput(mobileNetOutput, outputLayerName):
    with tf.name_scope(outputLayerName):
        w = tf.nn.conv2d(mobileNetOutput,weights(outputLayerName),[1,1,1,1],padding='SAME')
        w = tf.nn.bias_add(w,biases(outputLayerName))
    return w
...
def conv(inputs, stride, blockId):
    ...
    # w = tf.nn.relu6(w)
    # return w
    convLayer = f"Conv2d_{blockId}"

    with tf.name_scope(convLayer):
        w = tf.nn.conv2d(inputs,weights(convLayer), stride, padding='SAME')
        w = tf.nn.bias_add(w, biases(convLayer))
        w = tf.nn.relu6(w)
    return w
...
heatmaps = tf.identity(heatmaps, 'heatmap_2')
offsets = tf.identity(offsets, 'offset_2')
displacementFwd = tf.identity(displacementFwd, 'displacement_fwd_2')
displacementBwd = tf.identity(displacementBwd, 'displacement_bwd_2')
...
with tf.Session() as sess:
    ...
    if False:
        ...
        for i in range(heatmaps_result.shape[3]):
            plt.imsave(f'heatmap_{i}.png', heatmaps_result[0,:,:,i])
